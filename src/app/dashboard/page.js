      ...
      <div style={{ padding: '32px 20px', maxWidth: 800, margin: '0 auto' }}>
        ...
        <div className="animate-slide-up" style={{ marginBottom: 40 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
            ...
            <div>
              ...
              <h3 style={{ fontSize: 28, fontWeight: 800, color: 'white', letterSpacing: -0.5 }}>{user?.name || 'Student'}</h3>
            </div>
          </div>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12, marginBottom: 48 }}>
          {[
            ...
            { label: 'Total XP', val: stats.xp, icon: 'bolt', color: 'var(--primary)' },
            { label: 'Global Rank', val: stats.rank === 'Unranked' ? 'N/A' : `#${stats.rank}`, icon: 'workspace_premium', color: 'var(--accent)' }
          ].map((s, i) => (
